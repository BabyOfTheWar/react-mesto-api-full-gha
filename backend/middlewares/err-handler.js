...
const errorHandler = (err, req, res, next) => {
  ...
  if (err.name === 'ValidationError') {
    return res.status(constants.HTTP_STATUS.BAD_REQUEST)
      .json({ message: 'Неверный запрос' });
  }

  if (err.code === 11000) {
    return res.status(constants.HTTP_STATUS.CONFLICT)
      .json({ message: 'Пользователь с таким email уже существует' });
  }

  if (err.name === 'CastError' && err.kind === 'ObjectId') {
    return res.status(constants.HTTP_STATUS.BAD_REQUEST)
      .json({ message: 'Некорректный формат ID' });
  }
  ...
  if (err.name === 'nameLengthErr') {
    return res.status(constants.HTTP_STATUS.NOT_FOUND)
      .json({ message: 'Некорректная длина поля name' });
  }
